...
# Visualización mejorada
ggplot(agrupados_2013_muestra, aes(x = mass_category, y = count, fill = continent)) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.85, width = 0.9) +  
  scale_fill_brewer(palette = "Set1", name = "Continente") + 
...
Al observar los gráficos de distribución por continentes y clases de meteoritos:

Se identificaron clases específicas significativamente más frecuentes en ciertos continentes. Por ejemplo, en Asia y América del Norte se encontraron clases predominantes debido al mayor volumen de registros históricos y actividades de monitoreo.
En Europa y América del Norte, hay una diversidad relativamente alta de clases de meteoritos, probablemente reflejo de una investigación más detallada y recursos tecnológicos disponibles.  
*Agrupamientos geográficos:* ¿Existen regiones específicas donde se concentran ciertos tipos de meteoritos?
Se encontraron agrupamientos específicos en ciertas regiones, como una alta concentración de meteoritos pequeños en todos los continentes. Esto podría estar vinculado a la facilidad de detección de fragmentos menores o su mayor abundancia natural.
En la Antártida, se observa una mayor proporción de meteoritos más antiguos o raros, posiblemente porque el hielo preserva mejor sus características y facilita la búsqueda en un entorno aislado.
*Patrones inesperados:* ¿Hay alguna relación entre la clase del meteorito y su ubicación geográfica que no se esperaba? Algunos patrones revelaron que las clases menos comunes tienden a aparecer en regiones con climas extremos, como la Antártida. Esto sugiere un sesgo en la recolección de datos, ya que en estos lugares es más fácil encontrar meteoritos intactos debido a la menor actividad humana y la conservación natural.
Oceanía y África presentaron menos registros y diversidad de clases. Esto puede no reflejar la realidad geológica, sino la falta de estudios extensivos en estas regiones.
